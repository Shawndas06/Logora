...
def setup_flask_logging(app):
    ...
    file_handler = logging.handlers.RotatingFileHandler(
        filename=LOGS_DIR / "app.log", maxBytes=10 * 1024 * 1024, backupCount=5  # 10MB
    )
...
def log_report_generation(report_type, report_id=None, **kwargs):
    """Log report generation events."""
    from flask import current_app

    extra_info = " - ".join([f"{k}: {v}" for k, v in kwargs.items()])
    current_app.logger.info(
        f"Report Generation: {report_type} - ID: {report_id} - {extra_info}"
    )


def log_pdf_creation(pdf_path, report_id=None, **kwargs):
    """Log PDF creation events."""
    from flask import current_app

    extra_info = " - ".join([f"{k}: {v}" for k, v in kwargs.items()])
    current_app.logger.info(
        f"PDF Created: {pdf_path} - Report ID: {report_id} - {extra_info}"
    )
